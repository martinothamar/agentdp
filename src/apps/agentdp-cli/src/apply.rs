...
use agentdp_core::{Context, agent::AgentApplyResult, layout::AgentdpLayout, manifest::LoadedAgentManifest};
use agentdp_protocol::client_server::{AgentApplyParams, RequestKind};
use clap::Args;
...
async fn try_run(command: &Command, context: &Context) -> Result<(), Error> {
    ...
    let result: AgentApplyResult = server_client::request(
        context,
        &layout,
        RequestKind::AgentApply(AgentApplyParams {
            manifest: manifest.source_path().to_path_buf(),
        }),
        None,
    )
    .await
