...
use agentdp_core::{Context, agent::AgentScaleResult, layout::AgentdpLayout, manifest::LoadedAgentManifest};
use agentdp_protocol::client_server::{AgentScaleParams, RequestKind};
use clap::Args;
...
pub(crate) struct Command {
    #[arg(value_name = "REPLICAS", help = "Desired replica count")]
    pub replicas: u16,

    #[arg(short, long, value_name = "PATH", help = "Agent manifest path")]
    file: Option<PathBuf>,

    #[arg(long, help = "Wait until scaled replicas are stopped or ready")]
    wait: bool,
}
...
async fn try_run(command: &Command, context: &Context) -> Result<(), Error> {
    ...
    let result: AgentScaleResult = server_client::request(
        context,
        &layout,
        RequestKind::AgentScale(AgentScaleParams {
    ...
    println!("replicas: {}", result.status.replicas.desired);
    println!("ready replicas: {}", result.status.replicas.ready);
    ...
    if command.wait {
        ...
        let wait_result = wait::wait_for(
            ...
            result.generation(),
            wait::mutation_condition(&result),
            None,
            Some(&mut on_event),
        )
        .await
