...
mod tests {
    ...
    use super::{Logger, NoopSink};

    #[test]
    fn verbose_enabled_reflects_logger_configuration() {
        assert!(!Logger::quiet().verbose_enabled());
        assert!(!Logger::new(Arc::new(NoopSink), false).verbose_enabled());
        assert!(Logger::new(Arc::new(NoopSink), true).verbose_enabled());
    }
}
