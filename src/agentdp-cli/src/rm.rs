...
use agentdp_protocol::{InstanceRef, InstanceRmResult, RequestKind};
use clap::Args;
...
fn try_run(command: &Command, context: &Context) -> Result<(), Error> {
    ...
    let result: InstanceRmResult = server_client::request(
        context,
        paths,
        RequestKind::InstanceRm(InstanceRef {
            manifest,
