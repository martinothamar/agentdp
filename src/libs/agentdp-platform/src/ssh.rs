#![allow(
    clippy::future_not_send,
    clippy::large_futures,
    reason = "SSH command execution is driven from the server's local runtime and streams into local output sinks"
)]

use std::path::{Path, PathBuf};
use std::process::{Output, Stdio};
use std::sync::atomic::{AtomicU64, Ordering};
use std::time::Duration;

use thiserror::Error;
use tokio::io::{AsyncReadExt, AsyncSeekExt};
use tokio::process::Command;

pub const SSH_KEYGEN_PATH_ENV: &str = "AGENTDP_SSH_KEYGEN_PATH";
pub const SSH_PATH_ENV: &str = "AGENTDP_SSH_PATH";
pub const SSH_BINARY: &str = "ssh";
static TEMP_OUTPUT_COUNTER: AtomicU64 = AtomicU64::new(0);

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct KeyPair {
    pub private_key: PathBuf,
    pub public_key: PathBuf,
    pub public_key_contents: String,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConnectionInfo {
    pub user: String,
    pub host: String,
    pub port: u16,
    pub private_key: PathBuf,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CommandOutput {
    pub status: i32,
    pub stdout: String,
    pub stderr: String,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OutputStream {
    Stdout,
    Stderr,
}

pub trait OutputSink {
    fn output(&mut self, stream: OutputStream, chunk: &[u8]);
}

#[derive(Debug, Default)]
pub struct NoopOutputSink;

impl OutputSink for NoopOutputSink {
    fn output(&mut self, _stream: OutputStream, _chunk: &[u8]) {}
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CommandPrivilege {
    User,
    Root,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SshKeygen {
    binary: PathBuf,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SshClient {
    binary: PathBuf,
}

#[derive(Debug, Error)]
pub enum Error {
    #[error("ssh-keygen was not found; install ssh-keygen or set {SSH_KEYGEN_PATH_ENV}")]
    MissingSshKeygen,
    #[error("ssh was not found; install ssh or set {SSH_PATH_ENV}")]
    MissingSsh,
    #[error("failed to create SSH key directory {path}: {source}")]
    CreateDirectory {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("failed to remove stale SSH key {path}: {source}")]
    RemoveStaleKey {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("failed to restrict SSH private key permissions {path}: {source}")]
    RestrictPrivateKeyPermissions {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("failed to run ssh-keygen: {0}")]
    RunKeygen(#[source] std::io::Error),
    #[error("ssh-keygen failed: {stderr}")]
    KeygenFailed { stderr: String },
    #[error("failed to read generated SSH public key {path}: {source}")]
    ReadPublicKey {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("generated SSH public key {path} was empty")]
    EmptyPublicKey { path: PathBuf },
    #[error("failed to run ssh: {0}")]
    RunSsh(#[source] std::io::Error),
    #[error("guest command timed out after {timeout_seconds}s")]
    CommandTimedOut { timeout_seconds: u64 },
}

impl Error {
    #[must_use]
    pub const fn is_retryable(&self) -> bool {
        matches!(self, Self::CommandTimedOut { .. })
    }
}

impl SshKeygen {
    #[must_use]
    pub fn new(binary: impl Into<PathBuf>) -> Self {
        Self { binary: binary.into() }
    }

    /// Resolves the configured or PATH-discovered `ssh-keygen` executable.
    ///
    /// # Errors
    ///
    /// Returns an error when `AGENTDP_SSH_KEYGEN_PATH` is unset and
    /// `ssh-keygen` cannot be found on `PATH`.
    pub async fn resolve() -> Result<Self, Error> {
        if let Some(path) = std::env::var_os(SSH_KEYGEN_PATH_ENV).filter(|value| !value.is_empty()) {
            return Ok(Self::new(path));
        }
        let binary = super::host::find_binary("ssh-keygen")
            .await
            .ok_or(Error::MissingSshKeygen)?;
        Ok(Self::new(binary))
    }

    /// Generates an ed25519 key pair in `work_dir`.
    ///
    /// # Errors
    ///
    /// Returns an error when the key directory cannot be prepared,
    /// `ssh-keygen` cannot be started or exits unsuccessfully, or the generated
    /// public key cannot be read.
    pub async fn generate_key_pair(&self, work_dir: &Path) -> Result<KeyPair, Error> {
        let ssh_dir = work_dir.join("ssh");
        tokio::fs::create_dir_all(&ssh_dir)
            .await
            .map_err(|source| Error::CreateDirectory {
                path: ssh_dir.clone(),
                source,
            })?;
        let private_key = ssh_dir.join("agentdp_ed25519");
        let public_key = private_key.with_extension("pub");
        remove_stale_key(&private_key).await?;
        remove_stale_key(&public_key).await?;

        let mut command = Command::new(&self.binary);
        command
            .arg("-t")
            .arg("ed25519")
            .arg("-N")
            .arg("")
            .arg("-C")
            .arg("agentdp")
            .arg("-f")
            .arg(&private_key)
            .arg("-q");
        super::command::hide_child_window(&mut command);
        let output = command.output().await.map_err(Error::RunKeygen)?;
        if !output.status.success() {
            return Err(Error::KeygenFailed {
                stderr: String::from_utf8_lossy(&output.stderr).trim().to_owned(),
            });
        }
        super::fs::restrict_private_file_permissions(&private_key)
            .await
            .map_err(|source| Error::RestrictPrivateKeyPermissions {
                path: private_key.clone(),
                source,
            })?;

        let public_key_contents =
            tokio::fs::read_to_string(&public_key)
                .await
                .map_err(|source| Error::ReadPublicKey {
                    path: public_key.clone(),
                    source,
                })?;
        let public_key_contents = public_key_contents.trim().to_owned();
        if public_key_contents.is_empty() {
            return Err(Error::EmptyPublicKey { path: public_key });
        }

        Ok(KeyPair {
            private_key,
            public_key,
            public_key_contents,
        })
    }
}

impl SshClient {
    #[must_use]
    fn new(binary: impl Into<PathBuf>) -> Self {
        Self { binary: binary.into() }
    }

    /// Resolves the configured or PATH-discovered `ssh` executable.
    ///
    /// # Errors
    ///
    /// Returns an error when `AGENTDP_SSH_PATH` is unset and `ssh` cannot be
    /// found on `PATH`.
    pub async fn resolve() -> Result<Self, Error> {
        if let Some(path) = std::env::var_os(SSH_PATH_ENV).filter(|value| !value.is_empty()) {
            return Ok(Self::new(path));
        }
        let binary = super::host::find_binary(SSH_BINARY).await.ok_or(Error::MissingSsh)?;
        Ok(Self::new(binary))
    }

    /// Runs a command through SSH and waits for completion or timeout.
    ///
    /// # Errors
    ///
    /// Returns an error when `ssh` cannot be started, process polling fails, or
    /// the timeout elapses before the command exits.
    pub async fn run_command_with_timeout(
        &self,
        connection: &ConnectionInfo,
        command: &str,
        timeout: Duration,
        privilege: CommandPrivilege,
    ) -> Result<CommandOutput, Error> {
        let mut output = NoopOutputSink;
        self.run_remote_command_with_timeout(command_args(connection, command, privilege), timeout, &mut output)
            .await
    }

    /// Runs an already-quoted remote command through SSH.
    ///
    /// # Errors
    ///
    /// Returns an error when `ssh` cannot be started, process polling fails, or
    /// the timeout elapses before the command exits.
    pub async fn run_raw_command_with_timeout(
        &self,
        connection: &ConnectionInfo,
        command: &str,
        timeout: Duration,
    ) -> Result<CommandOutput, Error> {
        let mut output = NoopOutputSink;
        self.run_raw_command_with_timeout_and_output(connection, command, timeout, &mut output)
            .await
    }

    /// Runs an already-quoted remote command through SSH and streams output
    /// chunks as they arrive.
    ///
    /// # Errors
    ///
    /// Returns an error when `ssh` cannot be started, process polling fails, or
    /// the timeout elapses before the command exits.
    pub async fn run_raw_command_with_timeout_and_output(
        &self,
        connection: &ConnectionInfo,
        command: &str,
        timeout: Duration,
        output: &mut dyn OutputSink,
    ) -> Result<CommandOutput, Error> {
        self.run_remote_command_with_timeout(raw_command_args(connection, command), timeout, output)
            .await
    }

    async fn run_remote_command_with_timeout(
        &self,
        args: Vec<String>,
        timeout: Duration,
        output: &mut dyn OutputSink,
    ) -> Result<CommandOutput, Error> {
        let mut ssh_command = Command::new(&self.binary);
        ssh_command.args(args);
        super::command::hide_child_window(&mut ssh_command);
        let output = run_with_timeout(&mut ssh_command, timeout, output).await?;
        let status = output.status.code().unwrap_or(1);
        let stdout = String::from_utf8_lossy(&output.stdout).into_owned();
        let stderr = String::from_utf8_lossy(&output.stderr).into_owned();
        Ok(CommandOutput { status, stdout, stderr })
    }
}

#[must_use]
fn command_args(connection: &ConnectionInfo, command: &str, privilege: CommandPrivilege) -> Vec<String> {
    let mut args = base_args(connection);
    args.push(format!("{}@{}", connection.user, connection.host));
    match privilege {
        CommandPrivilege::User => args.push(format!("sh -lc {}", shell_single_quote(command))),
        CommandPrivilege::Root => args.push(format!("sudo -n sh -lc {}", shell_single_quote(command))),
    }
    args
}

#[must_use]
fn raw_command_args(connection: &ConnectionInfo, command: &str) -> Vec<String> {
    let mut args = base_args(connection);
    args.push(format!("{}@{}", connection.user, connection.host));
    args.push(command.to_owned());
    args
}

#[must_use]
pub fn shell_join(args: &[String]) -> String {
    args.iter()
        .map(|arg| shell_single_quote(arg))
        .collect::<Vec<_>>()
        .join(" ")
}

#[must_use]
pub fn interactive_shell_args(connection: &ConnectionInfo, remote_command: &str) -> Vec<String> {
    let mut args = base_args(connection);
    args.push("-t".to_owned());
    args.push(format!("{}@{}", connection.user, connection.host));
    args.push(remote_command.to_owned());
    args
}

fn base_args(connection: &ConnectionInfo) -> Vec<String> {
    vec![
        "-i".to_owned(),
        path_text(&connection.private_key),
        "-p".to_owned(),
        connection.port.to_string(),
        "-o".to_owned(),
        "BatchMode=yes".to_owned(),
        "-o".to_owned(),
        "IdentitiesOnly=yes".to_owned(),
        "-o".to_owned(),
        "ConnectTimeout=5".to_owned(),
        "-o".to_owned(),
        "StrictHostKeyChecking=no".to_owned(),
        "-o".to_owned(),
        format!("UserKnownHostsFile={}", null_device()),
        "-o".to_owned(),
        "LogLevel=ERROR".to_owned(),
    ]
}

const fn null_device() -> &'static str {
    if cfg!(target_os = "windows") {
        "NUL"
    } else {
        "/dev/null"
    }
}

async fn remove_stale_key(path: &Path) -> Result<(), Error> {
    match tokio::fs::remove_file(path).await {
        Ok(()) => Ok(()),
        Err(source) if source.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(source) => Err(Error::RemoveStaleKey {
            path: path.to_path_buf(),
            source,
        }),
    }
}

fn path_text(path: &Path) -> String {
    path.display().to_string()
}

fn shell_single_quote(value: &str) -> String {
    format!("'{}'", value.replace('\'', "'\"'\"'"))
}

async fn run_with_timeout(
    command: &mut Command,
    timeout: Duration,
    output: &mut dyn OutputSink,
) -> Result<Output, Error> {
    if cfg!(windows) {
        return run_with_file_output(command, timeout, output).await;
    }
    run_with_piped_output(command, timeout, output).await
}

async fn run_with_piped_output(
    command: &mut Command,
    timeout: Duration,
    output: &mut dyn OutputSink,
) -> Result<Output, Error> {
    command.kill_on_drop(true);
    let mut child = command
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .map_err(Error::RunSsh)?;
    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| Error::RunSsh(std::io::Error::other("ssh stdout pipe missing")))?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| Error::RunSsh(std::io::Error::other("ssh stderr pipe missing")))?;
    read_piped_output(timeout, child, stdout, stderr, output).await
}

async fn run_with_file_output(
    command: &mut Command,
    timeout: Duration,
    output: &mut dyn OutputSink,
) -> Result<Output, Error> {
    let stdout_path = temp_output_path("stdout");
    let stderr_path = temp_output_path("stderr");
    let result = run_with_file_output_paths(command, timeout, &stdout_path, &stderr_path, output).await;
    let _ = tokio::fs::remove_file(&stdout_path).await;
    let _ = tokio::fs::remove_file(&stderr_path).await;
    result
}

async fn run_with_file_output_paths(
    command: &mut Command,
    timeout: Duration,
    stdout_path: &Path,
    stderr_path: &Path,
    output: &mut dyn OutputSink,
) -> Result<Output, Error> {
    let stdout_file = tokio::fs::OpenOptions::new()
        .create_new(true)
        .write(true)
        .open(stdout_path)
        .await
        .map_err(Error::RunSsh)?
        .into_std()
        .await;
    let stderr_file = tokio::fs::OpenOptions::new()
        .create_new(true)
        .write(true)
        .open(stderr_path)
        .await
        .map_err(Error::RunSsh)?
        .into_std()
        .await;

    command.kill_on_drop(true);
    let mut child = command
        .stdin(Stdio::null())
        // Windows OpenSSH can hang forever after remote command completion when
        // stdout/stderr are inherited or anonymous pipes. Real files make the
        // child observe normal stdio handles and exit after channel close.
        .stdout(Stdio::from(stdout_file))
        .stderr(Stdio::from(stderr_file))
        .spawn()
        .map_err(Error::RunSsh)?;

    let mut stdout = FileOutput::new(stdout_path, OutputStream::Stdout);
    let mut stderr = FileOutput::new(stderr_path, OutputStream::Stderr);
    let deadline = command_timeout(timeout);
    let status = loop {
        stdout.read_new(output).await?;
        stderr.read_new(output).await?;
        if let Some(status) = child.try_wait().map_err(Error::RunSsh)? {
            break status;
        }
        if tokio::time::Instant::now() >= deadline {
            let _ = child.kill().await;
            return Err(Error::CommandTimedOut {
                timeout_seconds: timeout.as_secs(),
            });
        }
        tokio::time::sleep(Duration::from_millis(100)).await;
    };
    stdout.read_new(output).await?;
    stderr.read_new(output).await?;
    Ok(Output {
        status,
        stdout: stdout.bytes,
        stderr: stderr.bytes,
    })
}

fn temp_output_path(kind: &str) -> PathBuf {
    let id = TEMP_OUTPUT_COUNTER.fetch_add(1, Ordering::Relaxed);
    std::env::temp_dir().join(format!("agentdp-ssh-{kind}-{}-{id}.tmp", std::process::id()))
}

async fn read_piped_output(
    timeout: Duration,
    mut child: tokio::process::Child,
    mut stdout_pipe: tokio::process::ChildStdout,
    mut stderr_pipe: tokio::process::ChildStderr,
    output: &mut dyn OutputSink,
) -> Result<Output, Error> {
    let deadline = command_timeout(timeout);
    let mut status = None;
    let mut stdout_open = true;
    let mut stderr_open = true;
    let mut stdout = Vec::new();
    let mut stderr = Vec::new();
    let mut stdout_buffer = [0_u8; 8192];
    let mut stderr_buffer = [0_u8; 8192];
    loop {
        if let Some(status) = status
            && !stdout_open
            && !stderr_open
        {
            return Ok(Output { status, stdout, stderr });
        }
        tokio::select! {
            read = stdout_pipe.read(&mut stdout_buffer), if stdout_open => {
                let read = read.map_err(Error::RunSsh)?;
                if read == 0 {
                    stdout_open = false;
                } else {
                    let chunk = &stdout_buffer[..read];
                    stdout.extend_from_slice(chunk);
                    output.output(OutputStream::Stdout, chunk);
                }
            }
            read = stderr_pipe.read(&mut stderr_buffer), if stderr_open => {
                let read = read.map_err(Error::RunSsh)?;
                if read == 0 {
                    stderr_open = false;
                } else {
                    let chunk = &stderr_buffer[..read];
                    stderr.extend_from_slice(chunk);
                    output.output(OutputStream::Stderr, chunk);
                }
            }
            child_status = child.wait(), if status.is_none() => {
                let child_status = child_status.map_err(Error::RunSsh)?;
                status = Some(child_status);
            }
            () = tokio::time::sleep_until(deadline) => {
                let _ = child.kill().await;
                return Err(Error::CommandTimedOut {
                    timeout_seconds: timeout.as_secs(),
                });
            }
        }
    }
}

fn command_timeout(timeout: Duration) -> tokio::time::Instant {
    tokio::time::Instant::now() + timeout
}

struct FileOutput<'a> {
    path: &'a Path,
    stream: OutputStream,
    offset: u64,
    bytes: Vec<u8>,
}

impl<'a> FileOutput<'a> {
    const fn new(path: &'a Path, stream: OutputStream) -> Self {
        Self {
            path,
            stream,
            offset: 0,
            bytes: Vec::new(),
        }
    }

    async fn read_new(&mut self, output: &mut dyn OutputSink) -> Result<(), Error> {
        let mut file = tokio::fs::OpenOptions::new()
            .read(true)
            .open(self.path)
            .await
            .map_err(Error::RunSsh)?;
        file.seek(std::io::SeekFrom::Start(self.offset))
            .await
            .map_err(Error::RunSsh)?;
        let mut chunk = Vec::new();
        file.read_to_end(&mut chunk).await.map_err(Error::RunSsh)?;
        if chunk.is_empty() {
            return Ok(());
        }
        self.offset = self
            .offset
            .saturating_add(u64::try_from(chunk.len()).unwrap_or(u64::MAX));
        output.output(self.stream, &chunk);
        self.bytes.extend_from_slice(&chunk);
        Ok(())
    }
}

#[cfg(test)]
mod tests {
    use std::path::PathBuf;
    use std::time::Duration;

    use tokio::process::Command;

    use crate::ssh::{
        CommandPrivilege, ConnectionInfo, Error, OutputSink, OutputStream, command_args, interactive_shell_args,
        null_device, raw_command_args, run_with_timeout, shell_join,
    };

    #[test]
    fn builds_privileged_non_interactive_command_args() {
        let connection = connection();

        assert_eq!(
            command_args(&connection, "docker ps", CommandPrivilege::Root),
            [
                "-i",
                "/instances/pr-0/generated/qemu/ssh/agentdp_ed25519",
                "-p",
                "2222",
                "-o",
                "BatchMode=yes",
                "-o",
                "IdentitiesOnly=yes",
                "-o",
                "ConnectTimeout=5",
                "-o",
                "StrictHostKeyChecking=no",
                "-o",
                &format!("UserKnownHostsFile={}", null_device()),
                "-o",
                "LogLevel=ERROR",
                "arch@127.0.0.1",
                "sudo -n sh -lc 'docker ps'",
            ]
        );
    }

    #[test]
    fn quotes_guest_commands_for_remote_shell() {
        let connection = connection();

        assert_eq!(
            command_args(&connection, "printf '%s\\n' hello", CommandPrivilege::Root)
                .last()
                .unwrap(),
            "sudo -n sh -lc 'printf '\"'\"'%s\\n'\"'\"' hello'"
        );
    }

    #[test]
    fn builds_user_non_interactive_command_args() {
        let connection = connection();

        assert_eq!(
            command_args(&connection, "id -un", CommandPrivilege::User)
                .last()
                .unwrap(),
            "sh -lc 'id -un'"
        );
    }

    #[test]
    fn builds_raw_remote_command_args() {
        let connection = connection();

        assert_eq!(
            raw_command_args(&connection, "'/usr/local/bin/agentdp-agent-env' 'sh' '-lc' 'echo ok'"),
            [
                "-i",
                "/instances/pr-0/generated/qemu/ssh/agentdp_ed25519",
                "-p",
                "2222",
                "-o",
                "BatchMode=yes",
                "-o",
                "IdentitiesOnly=yes",
                "-o",
                "ConnectTimeout=5",
                "-o",
                "StrictHostKeyChecking=no",
                "-o",
                &format!("UserKnownHostsFile={}", null_device()),
                "-o",
                "LogLevel=ERROR",
                "arch@127.0.0.1",
                "'/usr/local/bin/agentdp-agent-env' 'sh' '-lc' 'echo ok'",
            ]
        );
    }

    #[test]
    fn builds_interactive_shell_args() {
        let connection = connection();

        assert_eq!(
            interactive_shell_args(
                &connection,
                "cd /data/home/code 2>/dev/null || cd; exec ${SHELL:-/bin/sh} -l",
            ),
            [
                "-i",
                "/instances/pr-0/generated/qemu/ssh/agentdp_ed25519",
                "-p",
                "2222",
                "-o",
                "BatchMode=yes",
                "-o",
                "IdentitiesOnly=yes",
                "-o",
                "ConnectTimeout=5",
                "-o",
                "StrictHostKeyChecking=no",
                "-o",
                &format!("UserKnownHostsFile={}", null_device()),
                "-o",
                "LogLevel=ERROR",
                "-t",
                "arch@127.0.0.1",
                "cd /data/home/code 2>/dev/null || cd; exec ${SHELL:-/bin/sh} -l",
            ]
        );
    }

    #[test]
    fn shell_join_quotes_command_arguments() {
        assert_eq!(
            shell_join(&[
                "printf".to_owned(),
                "%s\n".to_owned(),
                "hello world".to_owned(),
                "it's ok".to_owned(),
            ]),
            "'printf' '%s\n' 'hello world' 'it'\"'\"'s ok'"
        );
    }

    #[tokio::test]
    async fn streams_output_before_timeout_error() {
        let mut command = timeout_after_output_command();
        let mut output = CapturedOutput::default();

        let error = run_with_timeout(&mut command, Duration::from_secs(1), &mut output)
            .await
            .expect_err("command should time out");

        assert!(matches!(error, Error::CommandTimedOut { .. }));
        assert!(output.text(OutputStream::Stdout).contains("before-out"));
        assert!(output.text(OutputStream::Stderr).contains("before-err"));
    }

    #[tokio::test]
    async fn piped_output_returns_when_stdio_closes_before_process_exit() {
        let mut command = close_stdio_before_exit_command();
        let mut output = CapturedOutput::default();

        let result = tokio::time::timeout(
            Duration::from_secs(5),
            run_with_timeout(&mut command, Duration::from_secs(4), &mut output),
        )
        .await
        .expect("wait loop should not hang")
        .expect("child should exit");

        assert!(result.status.success());
    }

    fn timeout_after_output_command() -> Command {
        if cfg!(windows) {
            let mut command = Command::new("powershell.exe");
            command.args([
                "-NoProfile",
                "-Command",
                "[Console]::Out.WriteLine('before-out'); [Console]::Error.WriteLine('before-err'); Start-Sleep -Seconds 10",
            ]);
            return command;
        }
        let mut command = Command::new("sh");
        command.args(["-c", "printf 'before-out\\n'; printf 'before-err\\n' >&2; sleep 10"]);
        command
    }

    fn close_stdio_before_exit_command() -> Command {
        if cfg!(windows) {
            let mut command = Command::new("powershell.exe");
            command.args([
                "-NoProfile",
                "-Command",
                "[Console]::Out.Close(); [Console]::Error.Close(); Start-Sleep -Milliseconds 250",
            ]);
            return command;
        }
        let mut command = Command::new("sh");
        command.args(["-c", "exec >/dev/null 2>/dev/null; sleep 0.25"]);
        command
    }

    #[derive(Default)]
    struct CapturedOutput {
        chunks: Vec<(OutputStream, Vec<u8>)>,
    }

    impl CapturedOutput {
        fn text(&self, stream: OutputStream) -> String {
            String::from_utf8_lossy(
                &self
                    .chunks
                    .iter()
                    .filter(|(chunk_stream, _chunk)| *chunk_stream == stream)
                    .flat_map(|(_stream, chunk)| chunk.iter().copied())
                    .collect::<Vec<_>>(),
            )
            .into_owned()
        }
    }

    impl OutputSink for CapturedOutput {
        fn output(&mut self, stream: OutputStream, chunk: &[u8]) {
            self.chunks.push((stream, chunk.to_vec()));
        }
    }

    fn connection() -> ConnectionInfo {
        ConnectionInfo {
            user: "arch".to_owned(),
            host: "127.0.0.1".to_owned(),
            port: 2222,
            private_key: PathBuf::from("/instances/pr-0/generated/qemu/ssh/agentdp_ed25519"),
        }
    }
}
