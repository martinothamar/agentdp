...
/// Writes a FAT `CIDATA` cloud-init seed image asynchronously.
///
/// # Errors
///
/// Returns an error if the seed contents are too large or the image cannot be
/// written to disk.
pub async fn write(path: &Path, meta_data: &str, network_config: &str, user_data: &str) -> Result<(), Error> {
    write_with_extra_files(path, meta_data, network_config, user_data, &[]).await
}

/// Writes a FAT `CIDATA` cloud-init seed image with additional root files.
///
/// # Errors
///
/// Returns an error if the seed contents are too large or the image cannot be
/// written to disk.
pub async fn write_with_extra_files(
    path: &Path,
    meta_data: &str,
    network_config: &str,
    user_data: &str,
    ...
    let mut files = vec![
        ...
        SeedFile {
            long_name: "network-config",
            short_name: *b"NETWOR~1   ",
            contents: network_config.as_bytes(),
        },
        SeedFile {
            long_name: "user-data",
    ...
    files.extend(extra_files.iter().map(|file| SeedFile {
        long_name: file.long_name,
        short_name: file.short_name,
        contents: file.contents,
    }));
    let image = build_image(&files)?;
    tokio::fs::write(path, image).await.map_err(|source| Error::Write {
...
fn write_file_data(image: &mut [u8], layout: &Layout, files: &[SeedFile<'_>]) -> Vec<Vec<u16>> {
    ...
    for file in files {
        ...
        for chunk in file.contents.chunks(BYTES_PER_SECTOR * SECTORS_PER_CLUSTER) {
...
fn write_root_directory(
    ...
    for (file, chain) in files.iter().zip(chains) {
        ...
        for entry in lfn_entries(file.long_name, checksum)? {
            image[entry_offset..entry_offset + 32].copy_from_slice(&entry);
            entry_offset += 32;
        }
        write_short_entry(
...
fn lfn_entries(name: &str, checksum: u8) -> Result<Vec<[u8; 32]>, Error> {
    let mut name_units = name.encode_utf16().collect::<Vec<_>>();
    name_units.push(0);
    while name_units.len() % 13 != 0 {
        name_units.push(0xFFFF);
    }

    let chunks = name_units.chunks(13).collect::<Vec<_>>();
    if chunks.len() > 20 {
        return Err(Error::TooLarge);
    }
    ...
    for index in (0..chunks.len()).rev() {
        let order = u8::try_from(index + 1).map_err(|_| Error::TooLarge)?;
        let sequence = if index == chunks.len() - 1 { order | 0x40 } else { order };
        let mut entry = [0_u8; 32];
        write_lfn_entry(&mut entry, sequence, chunks[index], checksum);
        entries.push(entry);
    }
    Ok(entries)
}
...
fn write_lfn_entry(entry: &mut [u8], sequence: u8, name_units: &[u16], checksum: u8) {
    entry[0] = sequence;
    entry[11] = 0x0F;
    entry[13] = checksum;
    entry[26] = 0;
...
mod tests {
    ...
    use crate::seed_media::BYTES_PER_SECTOR;

    static NEXT_TEMP_ID: AtomicU64 = AtomicU64::new(0);

    #[tokio::test]
    async fn writes_cidata_fat_seed_image() {
        ...
        assert_eq!(image.len(), 32 * 1024 * 1024);
        ...
        assert!(contains_bytes(&image, b"version: 2\nethernets: {}\n"));
